...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            # ------------------------------------------------------------------
            if stop_sign_found:
                #! self._goal_state[2] = 0 원래 0아닌가? waypoint에는 0이 없네 ..
                #! 돌려보고 필요하면 넣기!
        ...
        # state.
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            self.state = STAY_STOPPED if closed_loop_speed < STOP_THRESHOLD else DECELERATE_TO_STOP
            # ------------------------------------------------------------------
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        next_index = wp_index + 1
        while next_index < len(waypoints) - 1:
            arc_length += math.sqrt(
                (waypoints[next_index][0] - waypoints[next_index-1][0]) ** 2 + (waypoints[next_index][1] - waypoints[next_index-1][1]) ** 2
            )
            if arc_length > self._lookahead:
                break
        # ------------------------------------------------------------------
        #! ego state를 사용을 안했는데 ... 써야하는거 아닌가 ..?
        return next_index
...
def get_closest_index(waypoints, ego_state):
    ...
    for index, wp in enumerate(waypoints):
        distance = math.sqrt((ego_state[0] - wp[0])**2 + (ego_state[1] - wp[1])**2)
